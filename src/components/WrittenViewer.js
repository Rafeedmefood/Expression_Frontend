...
function WrittenViewer() {
    const {id} = useParams();
    const data = useGetOperation(`https://expression-backend.herokuapp.com/api/written/${id}`)
                    ...
                    <div className="flex justify-center text-center mt-[5%]">
                        {data &&
                            <div key={data.id} className="w-[80%]">
                                <h2 className="home-titles font-Cinzel text-white mb-[30px] text-[35px]">{data.title}</h2>
                                <p className="text-white font-Akshar whitespace-pre-line mb-[5%]">
                                    {data.content}
                                </p>
